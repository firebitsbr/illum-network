...
*/
static unsigned char *illenc_decrypt(char *);
...
static char *illenc_byte2hex(unsigned char *bytes, int len)
{
	...
	hex = (char *)malloc(MAXTEXTSIZE * 2 + 1);
	memset(hex, '\0', MAXTEXTSIZE * 2 + 1);
...
static unsigned char *illenc_decrypt(char *hex)
{
	unsigned char *buffer = NULL, *bytes;
	unsigned int len;

	len = MAXTEXTSIZE + crypto_box_SEALBYTES;

	if (!hex || hex == NULL || strlen((char *)hex)
		> MAXTEXTSIZE * 2)
		return NULL;

	buffer = (unsigned char *)malloc(len + 1);
	bytes = illenc_hex2byte(hex);printf("%s\n======\n", bytes);
	memset(buffer, '\0', len);

	if (crypto_box_seal_open(buffer, bytes, len, pkeys->public,
		pkeys->secret) != 0)
		fprintf(errf, "Error: Can't decrypt message.\n");
...
static char *illenc_encrypt(unsigned char *text, char *ipaddr)
{
	...
	if (pkey != NULL) {
		...
		hex = illenc_byte2hex(buffer, MAXTEXTSIZE);
		printf("%s\n======\n", buffer);
	}
